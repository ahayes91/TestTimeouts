...
describe('Integration test at the app level: ', () => {
  it(`displays the bog details and the branches panel with no input and an unavailable message if branches are not available`, async () => {
    ...
    render(<App />);
    expect(
      await screen.findByText(/Branches are not available/),
    ).toBeInTheDocument(); // This is the final query the app makes to be fully loaded, so we use await/findBy - this fails intermittently in our CI
    expect(
